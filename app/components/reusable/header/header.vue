...
<template>
  <div>
    <!-- Mobile Sidebar -->
    <Dialog :open="sidebarOpen" @close="sidebarOpen = false" class="relative z-50 lg:hidden">
      <!-- Backdrop -->
      <DialogBackdrop class="fixed inset-0 bg-gray-900/80 transition-opacity duration-300 ease-linear" />

      <!-- Sidebar Panel -->
      <div class="fixed inset-0 flex">
        <DialogPanel class="relative flex w-full md:max-w-xs flex-1 transform transition duration-300 ease-in-out">
          <!-- Sidebar content -->
          <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-white py-[60px] px-6"   :style="{
    ...
    backgroundPosition: 'center'
            ...
            <div class="flex h-16 shrink-0 items-start justify-between">
                        <!-- Logo -->
                        <NuxtLink to="/">
                            <img :src="Logo" alt="Company Logo" class="w-full h-full" width="24" height="24" />
                        </NuxtLink>
